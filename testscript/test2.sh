...
DriveUrls () {
    ...
    redirectUris=$(jq -r '.clients[] | select(.clientId == "cdo-mondaine") | .redirectUris[0]' esdl-mapeditor-realm.json)
    ...
    sed -i "s|\"\(${redirectUris//\//\\/}\)\"|\"${DRIVE_REDIRECT_URI//\//\\/}\"|g" esdl-mapeditor-realm.json
    ...
    newredirectUris=$(jq -r '.clients[] | select(.clientId == "cdo-mondaine") | .redirectUris[0]' esdl-mapeditor-realm.json)
